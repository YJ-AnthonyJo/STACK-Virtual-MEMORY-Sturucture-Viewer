...
def set_(CMD):
    ...
    m = p.match(CMD)
    if bool(m): 
        vari1, vari2 = [m.group(i).rstrip() for i in [1,2]]
        ...
        if bool(m):
            ...
            byte = len(VARIABLES[vari2])
        
        if not chk_valid_variable_name(vari1, vari2): return #변수명 확인.
        
        
        if vari1 in VARIABLES: #기존에 존재시.
            print("This Variable name is already exist, please use other.")
            return
        else:
            VARIABLES[vari1] = VARIABLES[vari2][:byte]
        return
    ...
    m = p.match(CMD)
    if bool(m):
        vari1 = m.group(1).rstrip() 
        if not chk_valid_variable_name(vari1): return
        data = m.group(2)
        byte = m.group(3)
        byte = int(byte) if byte != '' else len(data)
        
        if vari1 in VARIABLES:
            print("This Variable name is already exist, please use other.")
            return
        VARIABLES[vari1] = data[:byte]
        return
    ...
    if bool(m): 
        ...
        # [ ['{vari}', '{data}', {byte}], ...]
        total = 0
        for idx, tmp in enumerate(STACK):
            _1, _2, stackByte = tmp
            total += stackByte
            if total == relativeAddr:
                STACK.insert(idx, [vari, VARIABLES[vari][:byte], byte])
        return
    ...
    if bool(m):
        ...
        for idx, tmp in enumerate(STACK):
            ...
            total += stackByte
            if total == relativeAddr:
                STACK[idx] = [vari, VARIABLES[vari][:byte], byte]
        return
